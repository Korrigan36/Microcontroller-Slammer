...
#        self.openFileNameDialog()
    ...
    def initUI(self):
        ...
        self.slewLabel = QLabel(self)
        self.slewLabel.setText("Slew Rate")
        sliderGrid.addWidget(self.slewLabel, 0, 0)

        self.slewRateSlider = QSlider(Qt.Horizontal, self)
        self.slewRateSlider.setFocusPolicy(Qt.NoFocus)
        self.slewRateSlider.setRange(1, 100)
        self.slewRateSlider.setValue(50)
        self.slewRateSlider.setGeometry(720, 150, 200, 20)
        self.slewRateSlider.setEnabled(True)
        self.slewRateSlider.sliderReleased.connect(self.slewRateChanged)

        self.slewLabel = QLabel(self)
        self.slewLabel.setText(str(self.slewRateSlider.value()) + "amps")
        sliderGrid.addWidget(self.slewLabel, 0, 1)

        sliderGrid.addWidget(self.slewRateSlider, 1, 0)
        
        self.slamLabel = QLabel(self)
        self.slamLabel.setText("Load Slam")
        sliderGrid.addWidget(self.slamLabel, 2, 0)

        self.slamLoadSlider = QSlider(Qt.Horizontal, self)
        self.slamLoadSlider.setFocusPolicy(Qt.NoFocus)
        self.slamLoadSlider.setRange(10, 90)
        ...
        sliderGrid.addWidget(self.slamLoad, 2, 1)

        sliderGrid.addWidget(self.slamLoadSlider, 3, 0)
        
        self.onTimeLabel = QLabel(self)
        self.onTimeLabel.setText("On Time")
        sliderGrid.addWidget(self.onTimeLabel, 4, 0)
        ...
        sliderGrid.addWidget(self.onTimeLabel, 4, 1)

        sliderGrid.addWidget(self.onTimeSlider, 5, 0)
        
        self.dutyLabel = QLabel(self)
        self.dutyLabel.setText("Duty Cycle")
        sliderGrid.addWidget(self.dutyLabel, 6, 0)
        ...
        sliderGrid.addWidget(self.dutyCycleLabel, 6, 1)

        sliderGrid.addWidget(self.dutyCycleSlider, 7, 0)
...
#        img = self.serialPort.read(200)
#        print(img)
#        self.serialPort.close()
#        time.sleep(1)
#        
        print ("before open serial")
        time.sleep(2)
...
#        value = pwm * 100
        value = pwm
        print ("PWM Value is: " + str(value))
        self.serialPort.write(struct.pack('L', int(value)))
        self.serialPort.flush()
        img = self.serialPort.read(80)
    ...
    def slewRateChanged(self):
        print ("Change Slam Load")
        slamLoad = self.slewRateSlider.value()        
        self.slamLoad.setText(str(self.slewRateSlider.value()) + "amps")
        load = slamLoad / 4.0
        volts = load * 0.0025
        ...
        port = "COM7"
        self.serialPort = serial.Serial(port, baudrate=9600, interCharTimeout=1)
        self.serialPort.write(str.encode("slew"))
        self.serialPort.flush()
        img = self.serialPort.read(80)
        print(img)
...
#        print ("orig val " + str(value))
        self.serialPort.write(struct.pack('L', self.onTime))
        self.serialPort.flush()
#        time.sleep(1)
        img = self.serialPort.read(80)
...
#
#        print ("orig val " + str(value))
        self.serialPort.write(struct.pack('L', self.dutyCycle))
        self.serialPort.flush()
#        time.sleep(1)
        img = self.serialPort.read(80)
        print("USB val " + str(img))
