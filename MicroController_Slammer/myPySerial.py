...
class Pyboard:
    def __init__(self, device, baudrate=9600, user='micro', password='python', wait=0):
...
#                    print (self.serial.portstr)       
    ...
    def write_Line(self, line):
        self.serial.write(str.encode(line))
    ...
    def hard_reset(self):
        self.serial.write(b'\r\x04\x04') # ctrl-C twice: interrupt any running program
    ...
    def exec_raw_no_waiting(self, command):
        if isinstance(command, bytes):
            command_bytes = command
        else:
            command_bytes = bytes(command, encoding='utf8')
...
#            raise PyboardError('could not exec command (response: %r)' % data)
    ...
    def exec_raw_no_wait(self, command, timeout=10, data_consumer=None):
        self.exec_raw_no_waiting(command);
        return 
...
#def execfile(filename, device='/dev/ttys11', baudrate=115200, user='micro', password='python'):
def execfile(filename, device='COM11', baudrate=115200, user='micro', password='python'):
...
def main():
    ...
    try:
        print ("here first")
        pyb = Pyboard('COM4', 9600, 'micro', 'python', 0)
        print ("now here")
    except PyboardError as er:
        print(er)
        sys.exit(1)
...
#        def execbuffer(buf):
    buf = {}
    buf[0] = """from pyb import Pin, DAC
import time
steps = 4
vout = 0.025
bitsPerVolt = 255.0 / 3.3
bits = int(bitsPerVolt * vout)
bitstring = str(bits)
print (bitstring)"""
    buf[1] = "dac = DAC(Pin('X5'))"
    buf[2] = "dac.write(0)"
    buf[3] = "pyb.LED(1).on()"
    buf[4] = "pyb.LED(2).on()"
    buf[5] = """for index in range(0,6):
    pyb.LED(3).on()  
    time.sleep(1)
    pyb.LED(3).off()
    time.sleep(1)
...
dac.write(0)"""
    ...
    for index in range(0,6):
        try:
            print (buf[index])
            ret, ret_err = pyb.exec_raw(buf[index], timeout=None, data_consumer=stdout_write_bytes)
        except PyboardError as er:
            print(er)
            pyb.close()
            sys.exit(1)
